...
interface IMEP1004 /*is IERC721*/ {
    ...
    function LocationProofs(uint256 _MEP1002TokenId, uint256[] memory _MEP1004TokenIds, bytes memory _item) external;

    // get the latest location proofs of anything.
    function latestLocationProofs(bytes memory _item) external view returns (LocationProof memory);
}
