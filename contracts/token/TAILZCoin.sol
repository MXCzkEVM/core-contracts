...
contract TAILZCoin is ControllableUpgradeable, ERC20PermitUpgradeable {
    ...
    function initialize(address _treasury, address _recipient, uint _defaultLPWANFee) external initializer {
        __Controllable_init();
        __ERC20_init("TAILZ Coin", "TAIL");
        __ERC20Permit_init("TAIL");
        treasury = _treasury;
        LPWANFee = _defaultLPWANFee;
        _mint(_recipient, 100000000 * (10 ** decimals()));

        _mint(_treasury, 900000000 * (10 ** decimals()));
...
contract ProxiedTAILZCoin is Proxied, UUPSUpgradeable, TAILZCoin  {
